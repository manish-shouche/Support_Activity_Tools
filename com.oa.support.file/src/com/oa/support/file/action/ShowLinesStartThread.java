 ...
 */
public class ShowLinesStartThread extends Thread {
	...
	public ShowLinesStartThread() {

	}

	public ShowLinesStartThread(FileHandlerAction action, byte[] arry, ActionDetails ad, byte[] lastThrdRemain) {
		this.act = action;
		this.actDetails = ad;
		this.bArray = arry;
	...
	public void run() {
		try {
			...
			while (lBBuffer.hasRemaining()) {
				...
				if (actDetails.getStartLine() >= act.linecount) {
					while (true) {
