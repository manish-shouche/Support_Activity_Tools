...
/**
 * This class executes the Find All in a file Action in a separate thread
 */
public class FindAllThread extends Thread {
	...
	public FindAllThread(FileHandlerAction action, byte[] arry, ActionDetails ad, byte[] lastThrdRemain) {
		this.act = action;
		this.actDetails = ad;
		this.bArray = arry;
	...
	public void run() {
		try {
			...
			while (lBBuffer.hasRemaining()) {
				...
				} else if (searchStrArr[matchIdx] == ch1 || searchStrArr[matchIdx] == (ch1 - 22)) {
					matchIdx++;
